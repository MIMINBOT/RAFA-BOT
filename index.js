/*
]=====> RAFA YTG <=====[ ]=====> YT RAFA YTG <=====[ ]=====> 15856662262 <=====[
*/
const {
   WAConnection,
   MessageType,
...
// JAN NGEYEL AJG
            + 'ORG:OWNER BOTZ;\n'
            + 'TEL;type=CELL;type=VOICE;waid=15856662262:+1 (585) 666-2262\n' 
            + 'END:VCARD' 
prefix = '!'
blocked = []   
limitawal = 20
memberlimit = 2
cr = '*SUBSCRIBE RAFA YTG*'

/*
]=====> SETTINGS <=====[
*/
const ownerNumber = ["15856662262@s.whatsapp.net","15856662262@s.whatsapp.net"]
const botName = 'RAFA YTG'
const ownerName = 'RAFA YTG'
...
*/
				...
				if (!isRegistered) return reply(ind.noregis())
				if (isLimit(sender)) return reply(ind.limitend(pusname))
				if (args.length < 1) return reply('Teksnya mana kak? Contoh : ${prefix}nulis1 RAFA YTG baik hati')
				ramlan = body.slice(8)
				...
				if (!isRegistered) return reply(ind.noregis())
				if (isLimit(sender)) return reply(ind.limitend(pusname))
				if (args.length < 1) return reply('Teksnya mana kak? Contoh : ${prefix}nulis2 RAFA YTG baik hati')
				laysha = body.slice(8)
				reply('「❗」WAIT BRO GUE NULIS DUMLU YAKAN')
				...
				client.sendMessage(from, ''+tes+'\n\n_By : ⸸RAFA YTG⸸Panutanque._', text, { quoted: mek })
				await limitAdd(sender)
				break
// NINJALOGO				
				case 'ninjalogo':
				...
				var nin = gh.split("&")[0];
				var ja = gh.split("&")[1];
				if (args.length < 1) return reply('「❗」Contoh : ${prefix}ninjalogo RAFA YTG & Gans')
				reply(ind.wait())
...
// PORNHUB
				...
				var porn = gh.split("&")[0];
				var hub = gh.split("&")[1];
				if (args.length < 1) return reply('「❗」Contoh : ${prefix}pornhub RAFA YTG & Hub')
				reply(ind.wait())
...
//GEMBOK TEXT
				...
				if (isLimit(sender)) return reply(ind.limitend(pusname))
					...
					var gem = gh.split("&")[0];
					var bok = gh.split("&")[1];
					if (args.length < 1) return reply('[❗] Contoh : ${prefix}gemboktext 11 01 2021 & RAFA YTG dan OTW 1RB SUBS')
					reply(ind.wait())
				...
				if (isLimit(sender)) return reply(ind.limitend(pusname))
					...
					var gli = gh.split("&")[0];
					var tch = gh.split("&")[1];
					if (args.length < 1) return reply('[❗] Contoh : ${prefix}glitchtext RAFA YTG & Gans')
					reply(ind.wait())
                ...
                var gh = body.slice(12)
					...
					var wm = gh.split("&")[1];
					const pref = `yang mau dijadiin quote apaan, titit?\n\ncontoh : ${prefix}bikinquote aku bukan boneka & Kata RAFA YTG`
...
*/				
            	case 'mining':
                      ...
                      if (!isEventon) return reply(`maaf ${pushname} event mining tidak di aktifkan sama owner RAFA YTG`)
                      if (isOwner) {
                      const one = 999999999
                      addLevelingXp(sender, one)
                      addLevelingLevel(sender, 99)
                      reply(`karena RAFA YTG baik Bot memberikan ${one}Xp >_<`)
                      }else{
            ...
            case 'creator':
                  ...
                  client.sendMessage(from, 'ITU NOMOR OWNER RAFA YTG [ PESAN DENGAN CARA YANG BAIK DAN SOPAN YA ☺ ]',MessageType.text, { quoted: mek} )
					break					
/*
]=====> MAKER MENU <=====[
*/
...
if (!isRegistered) return reply(ind.noregis())
if (isLimit(sender)) return reply(ind.limitend(pusname))
if (args.length < 1) return reply('「❗」Contoh : ${prefix}cloudtext RAFA YTG')
cloud = body.slice(11)
